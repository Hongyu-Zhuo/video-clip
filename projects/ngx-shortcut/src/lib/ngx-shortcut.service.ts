...
export class NgxShortcutService {
  ...
  triggerKeydown(e: KeyboardEvent) {
    ...
    for (const pair of this.activeCombinations) {
      for (const evt of pair.shortcut.events) {
          evt(pair.combination);
      }
  }
  }
  ...
  getCombinations(): ShortcutPair[] {
    ...
    for (const shortcut of this.shortcuts) {
        for (const combination of shortcut.combinations) {
            if (this.queueEndsWith(combination, combination.length)) {
                result.push({ shortcut, combination });
            }
        }
    }
    return result;
...
private queueEndsWith(arr: ShortcutKey[], checkLength: number): boolean {
    ...
    for (let i = 0; i < checkLength; i++) {
        if (this.activeKeys[matchStart + i] !== arr[i]) {
            return false;
        }
    }
    return true;
