...
@Directive({
  selector: '[vcDragSourceDragging]',
  exportAs: 'vcDragSourceDragging'
})
export class DragSourceDragging implements AfterContentInit, OnDestroy {
  isActive = false;

  @ContentChild(DragSourceDirective) source!: DragSourceDirective;

  @Input()
  set vcDragSourceDragging(data: string[] | string) {
  ...
  ngAfterContentInit(): void {
    ...
    this.subscription = this.source.dragging.subscribe((isDragging: boolean) => this.update(isDragging));
  }
